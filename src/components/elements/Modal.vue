    ...
    <div>
      <Transition name="modal-show">
        <div v-if="props.showModal"
          class="fixed w-4/6 top-16 left-2/4 -translate-x-1/2 rounded-lg border border-slate-800 shadow-lg z-50">
          <Teleport to="body">
            <div @click.stop="closeModal()"
              class="bg-white/30 backdrop-blur-sm fixed top-0 bottom-0 left-0 right-0 z-40 cursor-pointer"></div>
          </Teleport>
          ...
          <div class="px-4 py-3 bg-slate-100 max-h-[calc(100vh-16rem)] overflow-y-auto">
            <slot name="body"></slot>
          </div>
